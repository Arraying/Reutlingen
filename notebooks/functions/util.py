...
def plot_air_quality_forecast(city: str, street: str, df: pd.DataFrame, file_path: str, hindcast=False):
    ...
    ax.plot(day, df['predicted_pm25'], label='Vorhersage PM2.5', color='red', linewidth=2, marker='o', markersize=5, markerfacecolor='blue')
    ...
    ax.set_xlabel('Datum')
    ax.set_title(f"Reutlingen Zaisentalstraße PM2.5 Vorhersage (Logarithmische Skala)")
    ax.set_ylabel('PM2.5 (je weniger desto besser)')

    colors = ['green', 'yellow', 'orange', 'red', 'purple', 'darkred']
    labels = ['Gut', 'Mittelmäßig', 'Teilweise Ungesund', 'Ungesund', 'Sehr Ungesund', 'Gefährlich']
    ...
    legend1 = ax.legend(handles=patches, loc='upper right', title="Kategorien", fontsize='x-small')
